...
import { PlatformThemeTemplatesRepository } from '../platform-admin/theme-templates/platform-theme-templates.repository';
import { SeoService, type StorePageResponse } from '../seo/seo.service';
import { normalizeThemeConfig } from '../themes/utils/theme-config-normalizer';
...
export interface PublicStoreResolveResponse {
  ...
  publishedThemeSummary: {
    version: number;
    template: { id: string | null; renderer: string | null; componentKey: string | null };
  };
}
...
export class StorefrontService {
  constructor(
    ...
    private readonly platformThemeTemplatesRepository: PlatformThemeTemplatesRepository,
  ...
  async resolvePublicStore(request: Request): Promise<PublicStoreResolveResponse> {
    const store = await this.storeResolverService.resolve(request);
    const [publishedTheme, storeSettings, seoSettings] = await Promise.all([
      this.themesService.getStorefrontTheme(store.id),
    ...
    return {
      ...
      publishedThemeSummary: {
        ...
        template: this.extractPublishedTemplateSummary(publishedTheme.config),
      },
    };
  }
  ...
  async getTheme(request: Request, query: ThemeQueryDto) {
    const store = await this.storeResolverService.resolve(request);
    if (query.templatePreview && query.previewToken) {
      const preview = await this.platformThemeTemplatesRepository.findValidPreviewToken(
        query.previewToken,
        query.templatePreview,
      );
      if (!preview) {
        throw new NotFoundException('Template preview token is invalid or expired');
      }
      return {
        storeId: store.id,
        mode: 'preview',
        version: 0,
        config: normalizeThemeConfig(preview.config_snapshot),
      };
    }
  ...
  private extractPublishedTemplateSummary(config: Record<string, unknown>): {
    id: string | null;
    renderer: string | null;
    componentKey: string | null;
  } {
    const template = this.isRecord(config.template) ? config.template : {};
    return {
      id: typeof template.id === 'string' ? template.id : null,
      renderer: typeof template.renderer === 'string' ? template.renderer : null,
      componentKey: typeof template.componentKey === 'string' ? template.componentKey : null,
    };
  }
  ...
  private async prepareCheckoutData(
    ...
    if (fulfillment.availableMethods.length === 0) {
      throw new BadRequestException(
        'لا توجد طرق توصيل أو استلام متاحة حالياً لهذا المتجر. يرجى التواصل مع المتجر.',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException('طريقة التوصيل المحددة لم تعد متاحة. يرجى اختيار طريقة أخرى.');
    }
    ...
    if (requirePaymentMethod) {
      ...
      if (enabledPaymentMethods.length === 0) {
        throw new BadRequestException('لا توجد طرق دفع مفعلة لهذا المتجر حالياً.');
      }
    ...
    if (paymentSnapshot?.requiresReference && !payerReference) {
      throw new BadRequestException('رقم مرجع العملية مطلوب لطريقة الدفع المختارة.');
    }
  ...
  private async persistCheckoutTransaction(
    ...
    if (!checkoutData.shippingMethod || !checkoutData.shippingZone) {
      throw new BadRequestException('يرجى اختيار طريقة استلام الطلب قبل إكمال الطلب.');
    }
  ...
  private async saveCheckoutAddress(
    ...
    if (input.customerAddressId) {
      ...
      if (!selectedAddress) {
        throw new BadRequestException('العنوان المحدد غير موجود لهذا العميل.');
      }
      return;
    }
  ...
  private async resolveCheckoutFulfillment(
    ...
    if (methodId) {
      ...
      if (!selected) {
        throw new BadRequestException(
          'طريقة التوصيل المحددة لم تعد متاحة. يرجى اختيار طريقة أخرى.',
        );
      }
      if (zoneId && selected.shipping_zone_id !== zoneId) {
        throw new BadRequestException('طريقة التوصيل لا تتبع منطقة التوصيل المحددة.');
      }
      const zone = await this.resolveShippingZone(storeId, selected.shipping_zone_id);
      if (!zone) {
        throw new BadRequestException('التوصيل غير متاح لهذه المنطقة حالياً.');
      }
      return {
        zone,
