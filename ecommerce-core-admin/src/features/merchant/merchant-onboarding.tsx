...
const KALEEM_LOGO_SRC = '/brand/kaleem-logo.png';
const KALEEM_ICON_SRC = '/brand/kaleem-icon.png';
const KALEEM_SHAPE_SRC = '/brand/kaleem-bg-shape.png';
...
export function MerchantOnboarding({ session, onCompleted, onSignedOut }: MerchantOnboardingProps) {
  ...
  if (loading) {
    return (
      ...
      >
        <KaleemLoader size="lg" label="جاري تجهيز إعداد المتجر..." />
      </Box>
    );
  }
    ...
    <Box
      ...
      sx={{
        ...
        '&::after': {
          ...
          background: `url(${KALEEM_SHAPE_SRC}) center / contain no-repeat`,
            ...
            <Stack sx={{ position: 'relative', zIndex: 1, minHeight: '100%' }} spacing={3}>
              <Stack spacing={1.5}>
                ...
                <Typography sx={{ color: alpha('#fff', 0.78), lineHeight: 1.8 }}>
                  أكمل بيانات متجرك الأساسية، وسنجهز لك رابطًا فرعيًا واضحًا على نطاق كليم
                  ستورز.
                </Typography>
              </Stack>
...
function buildDefaultStoreUrl(slug: string): string {
  const pattern = import.meta.env.VITE_STOREFRONT_URL_PATTERN || 'https://{storeSlug}.kaleemstores.com';
