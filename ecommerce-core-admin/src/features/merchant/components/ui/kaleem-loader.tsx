...
type KaleemLoaderSize = 'sm' | 'md' | 'lg';

interface KaleemLoaderProps {
  size?: KaleemLoaderSize;
  label?: string;
...
const SIZE_MAP: Record<KaleemLoaderSize, { mark: number; dot: number; font: string }> = {
  sm: { mark: 34, dot: 5, font: '0.78rem' },
  md: { mark: 48, dot: 7, font: '0.86rem' },
  lg: { mark: 64, dot: 9, font: '0.94rem' },
};

export function KaleemLoader({
  size = 'md',
  label = 'جاري التحميل...',
  tone = 'primary',
  compact = false,
      ...
      >
        <Box
          sx={{
            ...
            animation: 'kaleem-loader-float 1.8s ease-in-out infinite',
          }}
        />
        <Box
          sx={{
            ...
            animation: 'kaleem-loader-pulse 1.8s ease-in-out infinite',
          }}
        />
        {[0, 1, 2].map((index) => (
          <Box
            ...
            sx={{
              ...
              animation: 'kaleem-loader-trail 1.35s ease-in-out infinite',
